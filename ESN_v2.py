...
class EchoStateNetwork:
    ...
    def copy_from(self,reservoir,bind=False):
        ...
        for attr_name,attr in reservoir.__dict__.items():
            if isinstance(attr,(int,float,NoneType,np.ufunc,np.vectorize,type(sigmoid))) or bind:
    ...
    def update_reservoir_layer(self,in_:np.ndarray=None,out_:np.ndarray=None,leak_version:int = 0,mode:Optional[str]=None):
        ...
        self.reservoir_layer = self._get_update(self.reservoir_layer,in_=in_,out_=out_,leak_version=leak_version_)
        ...
        # no u, yes y
            ...
            if leak_version:
                return (1-self.leak_rate)*x + \
                                self.f(self.leak_rate*np.dot( self.W, x ) + np.dot(self.Wback, at_least_2d(out_)))
            else:
                return (1-self.leak_rate)*x + \
                                self.leak_rate*self.f(np.dot( self.W, x ) + np.dot(self.Wback, at_least_2d(out_)))
        # yes u, no y
            ...
            if leak_version:
                if self.bias:
                    return (1-self.leak_rate)*x + \
                                self.f(self.leak_rate*np.dot( self.W, x ) + np.dot(self.Win, np.vstack((self._bias_vec,at_least_2d(in_)))))
                else:
                    return (1-self.leak_rate)*x + \
                                self.f(self.leak_rate*np.dot( self.W, x ) + np.dot(self.Win, at_least_2d(in_)))
            else:
                if self.bias:
                    return (1-self.leak_rate)*x + \
                                self.leak_rate*self.f(np.dot( self.W, x ) + np.dot(self.Win, np.vstack((self._bias_vec,at_least_2d(in_)))))
                else:
                    return (1-self.leak_rate)*x + \
                                self.leak_rate*self.f(np.dot( self.W, x ) + np.dot(self.Win, at_least_2d(in_)))
                ...
                if self.bias:
                    return (1-self.leak_rate)*x + \
                                self.f(self.leak_rate*np.dot( self.W, x ) + \
                                    np.dot(self.Win, np.vstack((self._bias_vec,at_least_2d(in_)))) + np.dot(self.Wback, at_least_2d(out_)))
                else:
                    return (1-self.leak_rate)*x + \
                                self.f(self.leak_rate*np.dot( self.W, x ) + \
                                    np.dot(self.Win, at_least_2d(in_)) + np.dot(self.Wback, at_least_2d(out_)))
    ...
    def __call__(self, in_):
        ...
        if self.bias:
            self._U = np.hstack((at_least_2d(in_).T,self.reservoir_layer.T,at_least_2d(np.array(self._bias_vec)))).T
        else:
            self._U = np.hstack((at_least_2d(in_).T,self.reservoir_layer.T)).T
        return np.dot(self.Wout,self._U)

        # Assuming it is already updated
        # self.update_reservoir_layer(in_)

        #if len(in_.shape)==self._input_shape_length:
            #if self._input_shape_length == 1:
                #return np.dot(self.Wout,np.vstack((self._bias_vec,at_least_2d(in_),self.reservoir_layer)))
            #elif self._input_shape_length == 2:
                #return np.dot(self.Wout,np.vstack((self._bias_vec,in_,self.reservoir_layer)))
        # else:
        #     raise Exception(f"Input has unsupported shape: {in_.shape}")

    def reset_reservoir_layer(self):
        self.reservoir_layer = self._reservoir_layer_init.copy()

    def copy_connections_from(self,reservoir,bind=False):
        assert isinstance(reservoir,(EchoStateNetwork,ESNX))
        for attr_name,attr in reservoir.__dict__.items():
            if ['Wout','W','Win','Wback'].count(attr_name):
...
class ESNX(EchoStateNetwork):
    ...
    def __init__(self, W: np.ndarray = None, 
        ...
        self.reservoir_layer = np.column_stack(batch_size*[self.reservoir_layer])#.T
        self._reservoir_layer_init = self.reservoir_layer.copy()
        # self._bias_vec = np.array(bool(self.bias)*batch_size*[self.bias])[:,None]
        self._bias_vec = bool(self.bias)*batch_size*[self.bias]
        #self._input_shape_length = 2
    ...
    #     # no u, no y
    #     if in_ is None and out_ is None:
    #         return (1-self.leak_rate)*x + self.leak_rate*self.f(np.dot( x, self.W.T ))    
    #     # no u, yes y
    #     elif in_ is None and out_ is not None:
    ...
    #         if leak_version:
    #             return (1-self.leak_rate)*x + \
    #                             self.f(self.leak_rate*np.dot(x, self.W.T) + np.dot(np.vstack((self._bias_vec,at_least_2d(in_))).T, self.Win.T))
    #         else:
    #             return (1-self.leak_rate)*x + \
    #                             self.leak_rate*self.f(np.dot(x, self.W.T) + np.dot(np.vstack((self._bias_vec,at_least_2d(in_))).T, self.Win.T ))
    ...
    #         # if leak_version:
    #         #     return (1-self.leak_rate)*x + \
    #         #                     self.f(self.leak_rate*np.dot(x, self.W) + \
    #         #                         np.dot(np.vstack((self._bias_vec,at_least_2d(in_))).T, self.Win) + np.dot(self.Wback, at_least_2d(out_)))
    #         # else:
    #         #     return (1-self.leak_rate)*x + \
    #         #                     self.leak_rate*self.f(np.dot(x, self.W) + \
    #         #                         np.dot(np.vstack((self._bias_vec,at_least_2d(in_))).T, self.Win) + np.dot(self.Wback, at_least_2d(out_)))
    ...
    #     self._U = np.hstack((at_least_2d(in_).T,self.reservoir_layer,self._bias_vec))
    #     return np.dot(self._U,self.Wout.T)
...
class ESNS(ESNX):
    ...
    def __init__(self, 
        ...
        assert no_of_reservoirs is not None and no_of_reservoirs>1

        self.no_of_reservoirs = no_of_reservoirs
        self.reservoir_layer = torch.tensor(no_of_reservoirs*[self.reservoir_layer.tolist()])
        self._reservoir_layer_init = self.reservoir_layer.copy()
        self._bias_vec = torch.tensor(no_of_reservoirs*[self._bias_vec.tolist()])